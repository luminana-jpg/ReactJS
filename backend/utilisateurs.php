...
switch ($methode) {
    ...
    case 'GET':
        if (!empty($_GET['id'])) {
            $stmt = $pdo->prepare('SELECT id, nom, email, role, statut, statut_session, date_creation FROM utilisateurs WHERE id = :id');
            ...
            $stmt = $pdo->query('SELECT id, nom, email, role, statut, statut_session, date_creation FROM utilisateurs ORDER BY id DESC');
